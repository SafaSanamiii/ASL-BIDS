function ExploreASL_Import_2(imPar, bCopySingleDicoms, bUseDCMTK, bCheckPermissions, bRunDCM2NII, x)
%ExploreASL_Import Imports the DICOM or PAR/REC raw data to NIFTIs
%
% FORMAT: ExploreASL_Import_safa(imPar[, bCopySingleDicoms, bUseDCMTK, bCheckPermissions, bRunDCM2NII, x])
%
% INPUT:
...
fprintf(['\nYou are using the legacy import workflow right now. With a future release,\n' ...
            ...
            'Also notice that this script was moved to the Outdated subfolder with v1.8.0.\n']);
fprintf('\n==============================================================================================\n\n');
...
if ~exist(imPar.RawRoot, 'dir')
    ...
    FolderNames = xASL_adm_GetFileList(fullfile(imPar.RawRoot), '^(?!(analysis|derivatives|source|raw)).*$', 'FPList', [0 Inf], true); 
...
new_param=strsplit(string(imPar.folderHierarchy),',');
new_param=cellstr(new_param);

[matches, tokens] = xASL_adm_FindByRegExp(imPar.RawRoot, new_param, 'StripRoot', true, 'Match', strLookFor,'IgnoreCase',true);
if isempty(matches)
